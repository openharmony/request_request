...
pub(crate) fn app_state_unavailable(uid: u64) -> String {
    format!(
        ...
        END,
            reason = CASE 
                WHEN (state = {WAITING} AND reason = {RUNNING_TASK_MEET_LIMITS} OR state = {RUNNING} OR state = {RETRYING}) THEN {APP_BACKGROUND_OR_TERMINATE} 
                ELSE reason
            END
        WHERE 
...
pub(crate) fn app_state_available(uid: u64) -> String {
    format!(
        "UPDATE request_task SET reason = {RUNNING_TASK_MEET_LIMITS} 
        WHERE 
            state = {WAITING} AND reason = {APP_BACKGROUND_OR_TERMINATE} AND uid = {uid}",
    )
}

pub(super) fn account_unavailable(active_accounts: &HashSet<u64>) -> String {
    let mut sql = format!(
        "UPDATE request_task SET state = {WAITING}, reason = {ACCOUNT_STOPPED} 
        WHERE 
            (state = {RUNNING} OR state = {RETRYING} OR (state = {WAITING} AND reason = {RUNNING_TASK_MEET_LIMITS})) AND uid/200000 NOT IN (",
    );
...
pub(super) fn account_available(active_accounts: &HashSet<u64>) -> String {
    let mut sql = format!(
        "UPDATE request_task SET reason = {RUNNING_TASK_MEET_LIMITS} 
        WHERE 
            (state = {WAITING} AND reason = {ACCOUNT_STOPPED}) AND uid/200000 IN (",
    );
...
pub(super) fn network_offline() -> String {
    format!(
        "UPDATE request_task SET 
            ...
            END,
            reason = CASE 
                WHEN (state = {WAITING} AND reason = {RUNNING_TASK_MEET_LIMITS} OR state = {RUNNING} OR state = {RETRYING}) THEN {NETWORK_OFFLINE} 
                ELSE reason
            END"
    )
}
...
pub(super) fn network_unavailable(info: &NetworkInfo) -> Option<String> {
    ...
    Some(format!(
        "UPDATE request_task SET 
            ...
            END,
            reason = CASE 
                WHEN (state = {WAITING} AND reason = {RUNNING_TASK_MEET_LIMITS} OR state = {RUNNING} OR state = {RETRYING}) THEN {UNSUPPORTED_NETWORK_TYPE} 
                ELSE reason
            END
        WHERE 
...
pub(super) fn network_available(info: &NetworkInfo) -> String {
    let mut sql = format!(
        "UPDATE request_task SET reason = {RUNNING_TASK_MEET_LIMITS} 
        WHERE 
            state = {WAITING} AND (reason = {NETWORK_OFFLINE} OR reason = {UNSUPPORTED_NETWORK_TYPE})",
    );
