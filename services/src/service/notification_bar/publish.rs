...
//!
//! This module provides functionality for dispatching various types of
//! notifications related to download and upload tasks, including progress
//! updates, success/failure notifications, and group management. It uses a
//! singleton pattern to maintain a central notification dispatcher.
...
///
/// This struct serves as a singleton and provides functionality to register
/// tasks, dispatch notifications, and manage notification groups for
...
impl NotificationDispatcher {
    ...
    /// # Arguments
    ///
    /// * `task_id` - ID of the task whose notification information should be
    ///   cleared
    ...
    ///
    /// This method helps maintain database size by removing outdated group
    ...
    pub(crate) fn get_task_gauge(&self, task_id: u32) -> Option<bool> {
        self.task_gauge
            ...
            .ok()?
            .get(&task_id)
            .map(|gauge| gauge.load(Ordering::Acquire))
    }

    /// Registers a task for notification tracking.
    ///
    /// Creates a notification visibility gauge for the task based on its group
    /// configuration or individual settings.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// Arc<AtomicBool> representing the notification visibility gauge for the
    /// task
    ...
    ///
    /// Stops notifications for the specified task and optionally affects group
    /// notifications.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// Creates and sends a progress notification with current task status
    /// information. Handles multi-file uploads and calculates total size
    /// when available.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// Removes the task from notification tracking and sends a success
    /// notification if notifications are available for this task.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// Removes the task from notification tracking and sends a failure
    /// notification if notifications are available for this task.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// Updates the notification gauge for all specified tasks to match the
    /// group's visibility setting and sends a notification to update the
    /// group.
    ///
    /// # Arguments
    ///
    ...
    pub(crate) fn attach_group(&self, task_ids: Vec<u32>, group_id: u32, uid: u64) -> bool {
        // Verify the caller owns this group; prevents cross-UID binding of one's
        // own task to another group to reuse its customized notification content
        // and visibility configuration.
        if !self.check_group_owner(group_id, uid) {
            return false;
        }
    ...
    pub(crate) fn delete_group(&self, group_id: u32, uid: u64) -> bool {
        ...
        // Only the group's creator may delete it; reject cross-UID deletion.
        if !self.check_group_owner(group_id, uid) {
            return false;
        }
    ...
    /// Checks whether the given UID owns the specified notification group.
    ///
    /// Ownership is established at group creation time and stored in the
    /// `group_notification_config.uid` column. Three cases are handled:
    /// - Group exists with a recorded owner: the owner must match `uid`.
    /// - Group exists but `uid` is NULL (legacy rows created before owner
    ///   tracking was introduced): the check is relaxed to `true` to keep
    ///   upgrade data usable. Only legacy rows are relaxed; newly created
    ///   groups are always owner-protected.
    /// - Group does not exist, or the database query fails: `false`.
    ///
    /// # Arguments
    ///
    /// * `group_id` - The ID of the group to check
    /// * `uid` - The UID to verify against the group's recorded owner
    ///
    /// # Returns
    ///
    /// `true` if the group exists and is either owned by `uid` or is a legacy
    /// owner-less row; `false` otherwise.
    pub(crate) fn check_group_owner(&self, group_id: u32, uid: u64) -> bool {
        match self.database.query_group_owner(group_id) {
            // New data: strict owner match.
            Some(owner) => owner == uid,
            // Legacy owner-less row (uid NULL) is relaxed to keep upgrade data
            // usable; a genuinely absent group fails the contains_group check.
            None => self.database.contains_group(group_id),
        }
    }

    /// Creates a new notification group with the specified configuration.
    ///
    /// Generates a unique group ID, stores the group configuration in the
    /// database, and optionally sets up customized notification text.
    ///
    /// # Arguments
    ///
    ...
    /// * `uid` - The UID of the calling process, recorded as the group owner
    ///
    /// # Returns
    ///
    ...
    pub(crate) fn create_group(
        ...
        info!(
            "Create group {} gauge {} customized_title {:?} customized_text {:?} want_agent {:?} disable {} visibility {} uid {}",
            new_group_id, gauge, title, text, want_agent, disable, visibility, uid
        );
        ...
        self.database.update_group_config(
            new_group_id,
            gauge,
            current_time,
            !disable,
        ...
        if title.is_some() || text.is_some() || want_agent.is_some() {
            self.database.update_group_customized_notification(
                new_group_id,
                title,
                text,
                want_agent,
