...
int32_t RequestCertManager::InitCertList(struct CertList **certList) {
  *certList = static_cast<struct CertList *>(malloc(sizeof(struct CertList)));
  ...
  (*certList)->certAbstract =
      static_cast<struct CertAbstract *>(malloc(buffSize));
...
struct CRequestCerts *RequestCertManager::GetUserCertsData() {
  ...
  if (ret != CM_SUCCESS) {
    REQUEST_HILOGE("GetUserCertsData, init cert list failed, ret = %{public}d",
                   ret);
    return nullptr;
  }
  ...
  if (ret != CM_SUCCESS) {
    REQUEST_HILOGE("GetUserCertsData, get cert list failed, ret = %{public}d",
                   ret);
  ...
  struct CRequestCerts *certs =
      static_cast<struct CRequestCerts *>(malloc(sizeof(struct CRequestCerts)));
  if (certs == nullptr) {
  ...
  certs->len = 0;
  struct CRequestCert **certDataList = static_cast<struct CRequestCert **>(
      malloc(MAX_COUNT_CERTIFICATE * sizeof(struct CRequestCert *)));
  ...
  for (uint32_t i = 0; i < certList->certsCount; i++) {
    (void)memset_s(&certInfo, sizeof(struct CertInfo), 0,
                   sizeof(struct CertInfo));
    ret = InitCertInfo(&certInfo);
    if (ret != CM_SUCCESS) {
      REQUEST_HILOGE(
          "GetUserCertsData, init cert info failed, ret = %{public}d ", ret);
    ...
    if (ret != CM_SUCCESS) {
      REQUEST_HILOGE(
          "GetUserCertsData, CmGetUserCertInfo failed, ret = %{public}d", ret);
    ...
    struct CRequestCert *cert =
        static_cast<struct CRequestCert *>(malloc(sizeof(struct CRequestCert)));
