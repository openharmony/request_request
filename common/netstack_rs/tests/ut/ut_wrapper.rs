...
mod tests {
    use super::*;
    use crate::{error::HttpErrorCode, info::DownloadInfoMgr};
    ...
    fn ut_task_status_try_from() {
        assert!(matches!(TaskStatus::try_from(ffi::TaskStatus::IDLE), Ok(TaskStatus::Idle)));
        assert!(matches!(TaskStatus::try_from(ffi::TaskStatus::RUNNING), Ok(TaskStatus::Running)));
    ...
    fn ut_response_code_try_from() {
        assert!(matches!(ResponseCode::try_from(ffi::ResponseCode::OK), Ok(ResponseCode::Ok)));
        assert!(matches!(ResponseCode::try_from(ffi::ResponseCode::NOT_FOUND), Ok(ResponseCode::NotFound)));
        assert!(matches!(ResponseCode::try_from(ffi::ResponseCode::INTERNAL_ERROR), Ok(ResponseCode::InternalError)));
    ...
    fn ut_http_error_code_try_from() {
        assert!(matches!(HttpErrorCode::try_from(ffi::HttpErrorCode::HTTP_NONE_ERR), Ok(HttpErrorCode::HttpNoneErr)));
        assert!(matches!(HttpErrorCode::try_from(ffi::HttpErrorCode::HTTP_WRITE_ERROR), Ok(HttpErrorCode::HttpWriteError)));
        assert!(matches!(HttpErrorCode::try_from(ffi::HttpErrorCode::HTTP_TASK_CANCELED), Ok(HttpErrorCode::HttpTaskCanceled)));
        
        // Test edge case with unknown error code
        let unknown_code = unsafe { std::mem::transmute(ffi::HttpErrorCode::HTTP_UNKNOWN_OTHER_ERROR) };
        assert!(matches!(HttpErrorCode::try_from(unknown_code), Ok(HttpErrorCode::HttpUnknownOtherError)));
    }
    ...
    fn create_test_wrapper_with_reset(callback: Box<dyn RequestCallback>, reset: Arc<AtomicBool>) -> CallbackWrapper {
        let task = Arc::new(Mutex::new(SharedPtr::null()));
        let task_weak = Arc::downgrade(&task);
        let task_id = TaskId::new();
        let info_mgr = Arc::new(DownloadInfoMgr::new());
