...
void HeaderReceiveCallback::HeaderReceive(const std::string &header)
{
    ...
    int ret = uv_queue_work(loop_, work,
        ...
        [](uv_work_t *work, int status) {
            ...
            jsHeader = UploadNapi::JSUtil::Convert2JSString(headerReceiveWorker->callback->env_,
                headerReceiveWorker->header);
            args[0] = { jsHeader };

            napi_get_reference_value(headerReceiveWorker->callback->env_,
                headerReceiveWorker->callback->callback_, &callback);
            napi_get_global(headerReceiveWorker->callback->env_, &global);
            callStatus = napi_call_function(
                headerReceiveWorker->callback->env_, global, callback, 1, args, &result);
