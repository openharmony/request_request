...
class DownloadTask {
public:
    DOWNLOAD_API explicit DownloadTask(uint32_t taskId);
    DOWNLOAD_API ~DownloadTask();

    DOWNLOAD_API uint32_t GetId() const;

    DOWNLOAD_API bool AddListener(const std::string &type, sptr<DownloadNotifyInterface> listener);
    void RemoveListener(const std::string &type, sptr<DownloadNotifyInterface> listener);
    DOWNLOAD_API void RemoveListener(const std::string &type);

    DOWNLOAD_API bool IsSupportType(const std::string &type);
