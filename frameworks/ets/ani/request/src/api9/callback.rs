...
//!
//! This module provides functions to register and manage callbacks for download
//! task events, including progress updates, completion, failure, pause, and
//! resume events. It implements a thread-safe callback management system using
//! a singleton pattern.
...
///
/// Adds a callback function that will be invoked when progress updates are
/// available for the specified download task.
///
/// # Parameters
///
...
///
/// Adds a callback function that will be invoked when a specific event occurs
/// for the specified download task.
///
/// # Parameters
///
/// * `env` - The animation environment reference
...
///
/// * `Ok(())` if the callback was successfully registered
/// * `Err(BusinessError)` if there was an error during registration or the
...
/// * The event type is not one of the supported values ("complete", "pause",
///   "resume")
///
/// # Examples
///
...
pub fn on_event(
    ...
    // Convert to global reference to ensure callback persists across function calls
    let callback = callback.into_global_callback(env).unwrap();
    info!("on_event called for task_id: {}, event: {}", task_id, event);
...
pub fn off_event(
    ...
    let task_id = this.task_id.parse().unwrap();
    info!(
        "off_event called for task_id: {}, event: {}",
        task_id, event
    );
    let callback_mgr = CallbackManager::get_instance();
    let callback = callback.into_global_callback(env).unwrap();
...
///
/// Stores and manages different types of callbacks for a download task,
...
///
/// Provides methods to handle different download events and execute the
/// corresponding registered callbacks with appropriate parameters.
impl request_client::Callback for CallbackColl {
    ...
    /// # Parameters
    ///
    /// * `progress` - The progress information containing processed bytes and
    ///   total size
    ...
    /// # Parameters
    ///
    /// * `_progress` - The final progress information (unused in this
    ///   implementation)
    ...
    /// # Parameters
    ///
    /// * `_progress` - The progress information at time of failure (unused in
    ///   this implementation)
    ...
    /// # Parameters
    ///
    /// * `_progress` - The progress information at time of pause (unused in
    ///   this implementation)
    ...
    /// # Parameters
    ///
    /// * `_progress` - The progress information at time of resume (unused in
    ///   this implementation)
...
///
/// Implements a singleton pattern to maintain a central registry of callback
/// collections indexed by task IDs.
